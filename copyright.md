...
## Ownership of Copyrights in R Consortium Project Contributions

When source code, documentation and other content is contributed to a R Consortium
project, the copyrights in those contributions remain owned by the original
copyright holders.

The copyrights are not _assigned_ to R Consortium. Instead, they are _licensed_ for
...
R Consortium does not require or recommend that every contributor include their
copyright notice in contributed files. [See below for more details on why
not.](#why-not-list-every-copyright-holder)

Instead, R Consortium recommends using a more general statement in a form similar to the
...
a recommended format for ease of use, but is not mandated by R Consortium.
